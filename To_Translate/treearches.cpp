...
const double ArchesTree::epsilon = 1e-6;
...
*/
ArchesTree::ArchesTree(TerraNode* r, TerraNode* w, TerraNode* s, TerraNode* f, VegetationNode* v)
{
  bedrock = r;
  water = w;
  sand = s;
...
/*!
\brief Deletes the tree structure.
*/
ArchesTree::~ArchesTree()
{
  delete bedrock;
  delete water;
  delete foam;
...
{
  if (water) return water->ElevationAlpha(p);
...
{
  if (foam) return foam->ElevationAlpha(p);
...
{
  if (sand) return sand->ElevationAlpha(p);
...
\param p Point.
*/
Vector ArchesTree::GetColor(const Vector2& p) const
{
  // Colors
...
{
  if (water) return bedrock->Traversal(p) + water->Traversal(p);
...
\brief \warn Todo
*/
Vector ArchesTree::dichotomie(Vector a, Vector b, double va, double vb) const
{
  double epsilon = 0.01;

  Vector start, middle, end;
  if (va > vb) { start = a; end = b; }
  else { start = b; end = a; }
...
*/
void ArchesTree::GetTriangles(Vector a, Vector b, Vector c, QVector<Vector> &triangleTerrain, QVector<Vector> &triangleWater) const {
